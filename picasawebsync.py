...
class Albums:
    ...
    def uploadMissingAlbumsAndFiles(self, compareattributes, mode, test, allowDelete):
        size = 0
        for album in sorted(self.albums.itervalues()):
        ...
        for album in sorted(self.albums.itervalues()):
            for file in sorted(album.entries.itervalues()):
    ...
    def upload_local(self, event):
        if self.type in chosenFormats:
            ...
            if self.type in supportedVideoFormats:
                if self.getLocalSize() > PICASA_MAX_VIDEO_SIZE_BYTES:
		    fflag = self.path + ".notuploadedtofgoogle"
		    touch(fflag)
...
immutableFolders = frozenset(["Instant Upload","Auto-Backup","Auto Backup","Automatische Sicherung","Sofort-Upload"])
...
def oauthLogin():
	...
	clsecrts = os.path.join(os.path.expanduser('~'), ".client_secrets.json")
	filename = os.path.join(os.path.expanduser('~'), ".picasawebsync")
	storage = Storage(filename)
	credentials = storage.get()
	if credentials is None or credentials.invalid:	
		flow = client.flow_from_clientsecrets(clsecrts,scope='https://picasaweb.google.com/data/',redirect_uri='urn:ietf:wg:oauth:2.0:oob')	
